...
#include <stdint.h>
#include <string.h>
#include "zf_driver_spi.h"        // SeekFree 硬件 SPI 驱动
#include "zf_driver_gpio.h"       // GPIO 控制（手动 CS）
...
typedef struct
{
    ...
    uint16_t    address;             // 未用（SPI 不需要 I2C 地址），保留兼容
    uint8_t     spi_n;               // SPI 索引: SPI_0 / SPI_1 / SPI_3
    gpio_pin_enum cs_pin;            // 手动 CS 引脚
    gpio_pin_enum xshut_pin;         // XSHUT/LPn 引脚
    // 写缓冲区 — 最大传输 = 4096 块 + 2 字节地址
    uint8_t     wr_buffer[4098];
